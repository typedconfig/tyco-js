...
const BASIC_STR_ESCAPE_REPLACEMENTS: Record<string, string> = {
  '\\b': '\u0008',  // backspace
  '\\t': '\u0009',  // tab
  '\\n': '\u000A',  // linefeed
  '\\f': '\u000C',  // form feed
...
function subEscapeSequences(content: string): string {
  ...
  let escaped = content;
  for (const [pattern, replacement] of Object.entries(BASIC_STR_ESCAPE_REPLACEMENTS)) {
    escaped = escaped.replace(new RegExp(pattern.replace(/\\/g, '\\\\'), 'g'), replacement);
  }
  
  // Replace Unicode escape sequences \uXXXX and \UXXXXXXXX
  escaped = escaped.replace(/\\u([0-9a-fA-F]{4})|\\U([0-9a-fA-F]{8})/g, (match, u4, u8) => {
    const hex = u4 || u8;
    return String.fromCharCode(parseInt(hex, 16));
  });
  
  // Remove escaped newlines with trailing whitespace
...
class TycoLexer {
  ...
  private loadSchema(struct: TycoStruct): void {
    ...
    while (true) {
      ...
      if (options === '*') {
        if (isArray) {
          throw new Error('Can not set a primary key on an array');
        }
...
class TycoValue {
  ...
  renderBaseContent(): void {
    ...
    if (this.isNullable && content === 'null') {
      ...
      let base = 10;
      if (content.startsWith('0x')) base = 16;
      else if (content.startsWith('0o')) base = 8;
      else if (content.startsWith('0b')) base = 2;
      rendered = parseInt(content, base);
      ...
      // Normalize datetime format from "YYYY-MM-DD HH:MM:SSZ" to "YYYY-MM-DDTHH:MM:SS+00:00"
...
export function loads(content: string): any {
  const context = new TycoContext();
  const lines = content.split(/\r?\n/).map(line => line.endsWith('\n') ? line : line + EOL);
